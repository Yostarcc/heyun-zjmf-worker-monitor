...
3. 准备 Cloudflare Token、Account ID、Fork 后的仓库地址和魔方财务 API；如需网页一键更新，再准备 GitHub Fine-grained Token。
...
`WEB_UPDATE_GITHUB_TOKEN` 不填时，管理后台仍可检查更新；点击“确定更新”会提示 `GITHUB_TOKEN_NOT_CONFIGURED`。要启用网页一键更新，请给 GitHub Fine-grained Token 授予当前 Fork 仓库 **Actions: Read and write**、**Contents: Read-only** 权限。
